...
新しいサーバーインスタンスを作成する
先ほど説明した基本的なサーバーライフサイクルは、新しいサーバーインスタンスの作成から始まります。もっと詳細なライフサイクルには、サーバーインスタンスを作成するためのサーバーイメージの作成と更新の手順が含まれますが、それについては別の章（第 13 章）で説明します。今のところ、我々のライフサイクルはインスタンスの作成とともに始まります（図 11-4 参照）。図 11-4 サーバーインスタンスの作成
...
- オペレーティングシステムと初期ソフトウェアをインストールします。オペレーティングシステムは、AMI のようなサーバーイメージからディスクボリュームにコピーする場合があります。または、新しいインスタンスがスクリプト可能なインストーラーを使用してファイルを選択してコピーするインストールプロセスを実行する場合もあります。スクリプト化された OS インストーラーの例には、Red Hat Kickstart、Solaris JumpStart、Debian Preseed、Windows インストール回答ファイルなどがあります。
...
- サーバーインスタンスをサービスに登録します。例えば、新しいサーバーを監視システムに追加します。これらは相互に排他的ではないため、サーバーの作成プロセスではこれらのさまざまなアクティビティを実行する 1 つ以上のメソッドを使用する場合があります。

サーバーインスタンスの作成をトリガーするために使用できるいくつかのメカニズムがあります。それぞれのメカニズムについて説明しましょう。

新しいサーバーインスタンスを手動で作成する
インフラストラクチャプラットフォームは、通常、Web UI、コマンドラインツール、GUI アプリケーションなど、新しいサーバーを作成するためのツールを提供します。新しいサーバーを作成するたびに、ソースイメージ、割り当てるリソース、ネットワーキングの詳細など、必要なオプションを選択します。
...
プラットフォームを試行するために UI やコマンドラインツールを使うことは役立ちますが、人々が必要とするサーバーを作成する良い方法ではありません。以前に説明したスタックの作成と構成の原則（「スタックの構成パターン」を参照）は、サーバーにも適用されます。手動でオプションを設定する（「アンチパターン：手動スタックパラメータ」で説明）と、ミスが生じやすく、設定が一貫しておらず、予測できないシステムと多くのメンテナンス作業を引き起こします。

スクリプトを使用してサーバーを作成する
サーバーを一貫して作成するためのスクリプトを作成することができます。スクリプトは、コマンドラインツールをラップするか、スクリプトコードや設定ファイルで構成オプションを設定するためにインフラストラクチャプラットフォームの API を使用します。サーバーを作成するスクリプトは再利用可能で一貫性があり、透明です。これは、スタックのスクリプト化されたパラメータパターンと同じコンセプトです。
...
このスクリプトは、前のコマンドラインの例と同じです。しかし、スクリプト内にあるため、私のチームの他のメンバーが私がどのようにサーバーを作成したかを見ることができます。彼らもより多くのサーバーを作成し、自分が作成したものと同じ動作をすることを確信することができます。Terraform や他のスタック管理ツールが登場する前は、私が働いていたほとんどのチームは、サーバーを作成するためのスクリプトを書いていました。通常、我々はこれらのスクリプトを設定ファイルで設定可能にしました（スタック構成パターンと同様）。しかし、これらのスクリプトの改善や修正に時間を費やしすぎました。

サーバーを作成するためのスタック管理ツールを使用する
第 5 章で説明したようなスタック管理ツールを使用すると、サーバーを他のインフラストラクチャリソースのコンテキストで定義することができます（例 11-1 参照）。このツールは、プラットフォームの API を使用してサーバーインスタンスを作成または更新します。
...
サーバーを作成するためにスタックツールを使用する利点はいくつかあります。一つは、ツールがエラーチェックのような、スクリプトで実装しなければならないロジックを自動で処理してくれることです。また、スタックは他のインフラストラクチャ要素との統合も処理してくれます。たとえば、スタック内で定義されたネットワーク構造やストレージとサーバーの接続を行います。例 11-1 のコードは、変数${APPSERVER_VNET}を使用して vnet パラメータを設定しています。これはおそらく、スタックコードの他の部分で定義されたネットワーク構造を指しています。プラットフォームを自動的にサーバーを作成できるように設定するほとんどのインフラストラクチャプラットフォームは、特定の条件下で新しいサーバーインスタンスを自動的に作成できます。一般的な場合は、オートスケーリング（負荷が増えたときにサーバーを追加する）とオートリカバリ（サーバーインスタンスが失敗した場合に置き換える）です。このような設定は、例 11-2 のようにスタックコードで通常定義できます。例 11-2 では、プラットフォームに対して最低 2 つ、最大 5 つのサーバーインスタンスを保持するよう指示しています。プラットフォームは必要に応じてサーバーインスタンスを追加または削除し、それらの CPU 利用率を 40%に近づけます。また、定義にはヘルスチェックも含まれています。このチェックはポート 8443 の/health に HTTP リクエストを行い、リクエストが 200 の HTTP レスポンスコードを返す場合には、サーバーが正常であるとみなします。サーバーが予想されたコードを 90 秒以内に返さない場合、プラットフォームはサーバーが失敗したとみなし、新しいインスタンスを作成して破棄します。サーバーを構築するためのネットワークプロビジョニングツールを使用する チャプター 3 で、ハードウェアサーバーをダイナミックにプロビジョニングするベアメタルクラウドについて説明しました。これを行うためのプロセスには、通常次の手順が含まれます。未使用の物理サーバーを選択し、サーバーファームウェアでサポートされている「ネットワークインストール」モード（例）を起動させる。ネットワークインストーラーは、シンプルなブートストラップ OS イメージからサーバーを起動して OS のインストールを開始します。OS のインストールイメージをダウンロードし、主要なハードドライブにコピーします。サーバーを再起動してセットアップモードで OS を起動し、アンアテンディッドスクリプトの OS インストーラーを実行します。このプロセスを管理するための多数のツールがあります。Crowbar、Cobbler、FAI（完全自動インストール）、Foreman、MAAS、Rebar、Tinkerbell などがあります。OS インストールイメージではなく、準備されたサーバーイメージを起動することもできます。これを行うことで、次のセクションで説明するサーバーの準備方法の一部を実装する機会が生まれます。
FaaS イベントを使用してサーバーをプロビジョニングする FaaS サーバーレスコードは、新しいサーバーのプロビジョニングに関与することができます。プラットフォームは、新しいインスタンスが作成される前、作成中、作成後のさまざまなタイミングでコードを実行できます。例えば、新しいサーバーにセキュリティポリシーを割り当てたり、監視サービスに登録したり、サーバーコンフィギュレーションツールを実行したりすることができます。
